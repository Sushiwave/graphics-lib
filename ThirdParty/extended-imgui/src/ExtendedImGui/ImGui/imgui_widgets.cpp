...
#endif
#include <ExtendedImGui/ImGui/imgui_internal.h>
    ...
    if (is_logarithmic)
    {
        ...
        else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
        {
            ...
            if (v == 0.0f)
                ...
                result = (1.0f - (float)(ImLogEX(-(FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / ImLogEX(-v_min_fudged / logarithmic_zero_epsilon))) * zero_point_snap_L;
            else
                result = zero_point_snap_R + ((float)(ImLogEX((FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / ImLogEX(v_max_fudged / logarithmic_zero_epsilon)) * (1.0f - zero_point_snap_R));
        }
        else if ((v_min < 0.0f) || (v_max < 0.0f)) // Entirely negative slider
            result = 1.0f - (float)(ImLogEX(-(FLOATTYPE)v_clamped / -v_max_fudged) / ImLogEX(-v_min_fudged / -v_max_fudged));
        else
            result = (float)(ImLogEX((FLOATTYPE)v_clamped / v_min_fudged) / ImLogEX(v_max_fudged / v_min_fudged));
...
#include <ExtendedImGui/ImGui/imstb_textedit.h>
