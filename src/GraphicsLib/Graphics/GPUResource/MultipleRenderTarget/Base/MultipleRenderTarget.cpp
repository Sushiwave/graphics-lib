...
namespace cg
{
	MultipleRenderTarget::MultipleRenderTarget(const SetCall& setCall, const std::vector<std::shared_ptr<IRenderTarget>>& renderTargetList)
	...
	void MultipleRenderTarget::set(std::shared_ptr<IDepthStencilBuffer> depthStencilBuffer)
	{
		auto selfID = getID();
	...
	cpp::Vector2D<int> MultipleRenderTarget::getRenderTargetSize(int index) const
	{
		return m_renderTargetList.at(index)->getSize();
	}

	std::shared_ptr<ITexture2D> MultipleRenderTarget::getRenderingResult(int index)
	{
		return m_renderTargetList.at(index)->getRenderingResult();
	}

	int MultipleRenderTarget::getRenderTargetCount() const noexcept
	{
		return m_count;
	}

	void MultipleRenderTarget::refresh(int unit)
	{
		m_renderTargetList.at(unit)->refresh();
	}
	void MultipleRenderTarget::refreshAll()
	{
		for (auto renderTarget : m_renderTargetList)
		{
